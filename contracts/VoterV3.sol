...
contract VoterV3 is IVoter, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setVotingEscrow(address __ve) external {
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) {
            // gov can create for any pool, even non-Custo pairs
        ...
        _type = string.concat("Custo Bribes: ", IERC20(_pool).symbol());
    ...
    function initGauges(address[] memory _gauges, address[] memory _pools)
        public
    {
        require(msg.sender == emergencyCouncil);
        uint256 i = 0;
