...
contract AirdropClaimCusNFT is ReentrancyGuard {
    ...
    /// @notice claim the given amount and send to _to. Checks are done by merkle tree contract. (eg.: 40% veCUS 60% $CUS)
    function claim(address _who, uint _amount, address _to) external nonReentrant onlyMerkle returns(bool){
        ...
        uint256 _tokenId = IVotingEscrow(ve).createLockFor(_veShare, LOCK_PERIOD, _to);
        require(_tokenId != 0);
