...
contract MinterUpgradeable is IMinter, OwnableUpgradeable {
    ...
    uint256 public weekly; // represents a starting weekly emission of 2.6M Token (Token has 18 decimals)
    ...
    function initialize(
        ...
        _baseToken = IBaseToken(IVotingEscrow(__ve).token());
        _voter = IVoter(__voter);
        _ve = IVotingEscrow(__ve);
        _rewardsDistributor = IRewardsDistributor(__rewardsDistributor);

        activePeriod = ((block.timestamp + (2 * WEEK)) / WEEK) * WEEK;
        weekly = 2_600_000 * 1e18; // represents a starting weekly emission of 2.6M CUS (CUS has 18 decimals)
        isFirstMint = true;
    }
    ...
    function _initialize(
        ...
        if (max > 0) {
            _baseToken.mint(address(this), max);
            _baseToken.approve(address(_ve), type(uint256).max);
    ...
    function circulatingSupply() public view returns (uint256) {
        return
            _baseToken.totalSupply() - _baseToken.balanceOf(address(_ve));
    }
    ...
    function calculateRebate(uint256 _weeklyMint)
        ...
        returns (uint256)
    {
        uint256 _veTotal = _baseToken.balanceOf(address(_ve));
        uint256 _baseTokenTotal = _baseToken.totalSupply();

        uint256 lockedShare = ((_veTotal) * PRECISION) / _baseTokenTotal;
    ...
    function updatePeriod() external returns (uint256) {
        ...
        if (block.timestamp >= _period + WEEK && _initializer == address(0)) {
            ...
            uint256 _balanceOf = _baseToken.balanceOf(address(this));
            if (_balanceOf < _required) {
                _baseToken.mint(address(this), _required - _balanceOf);
            }

            require(_baseToken.transfer(team, _teamEmissions));

            require(
                _baseToken.transfer(address(_rewardsDistributor), _rebase)
            );
            ...
            _baseToken.approve(address(_voter), _gauge);
