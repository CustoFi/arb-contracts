...
contract Royalties is ReentrancyGuard {
    ...
    constructor(address _wbnb, address _custodian) {
        owner = msg.sender;
        wbnb = IERC20(_wbnb);
    ...
    function deposit(uint256 amount) external payable allowed {
        ...
        totalSupply[epoch] = custodian.totalSupply();
        reservedAmounts[epoch] = custodian.reservedAmount();
        epoch++;
    }
    ...
    function claimable(address user) public view returns(uint) {
        ...
        //Total fees * Custodian.originalMinters[msg.sender] / (Custodian.totalSupply - Custodian.reservedAmount)
        ...
        for(i = cp; i < epoch; i++){
            ...
            uint256 weight = custodian.originalMinters(user);
