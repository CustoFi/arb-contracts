...
async function main() {
    ...
    const theContract = await deployContract("BaseToken", []);
    await sendTxn(theContract.mint(process.env.PUBLICKEY, ethers.utils.parseUnits("1000000000", 18)), "custoContract.mint");

    //Mint theNFT
    const custodianContract = await deployContract("custodian", [ 3000, ethers.utils.parseUnits("25", 15), timestamp - 2 * 24 * 60 * 60 - 1, process.env.PUBLICKEY ]);
    await sendTxn(custodianContract.reserveNFTs(process.env.PUBLICKEY, 5), "custodianContract.reserveNFTs");
    await sendTxn(custodianContract.mintPublic(5, {
        value: ethers.utils.parseUnits("125", 15),
        gasLimit: "3000000",
    }), "custodianContract.mintPublic");
    ...
    const airdropClaimNftContract = await deployContract("AirdropClaimCusNFT", [ theContract.address, veContract.address ]);

    await deployContract("MerkleTreeCusNFT", [ airdropClaimNftContract.address ]);
